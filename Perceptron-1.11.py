...
class NeuralNetwork(object):
    instance=0
    def __init__(self, inputs=2, hidden1=12, hidden2=11, outputs=1, bias=3, nList=[ [], [], [], [], [] ]):
    ...
    for i in range(len(wList)):
        for j in range(len(wList[i])):
            if type(wList[i][j]) == float or type(wList[i][j]) == int:
                wList[i][j] = round(wList[i][j] + random.choice([0.0,0.0,0.0,random.uniform(-1,1),random.uniform(-1,1)*error]),3)
            else:
                for k in range(len(wList[i][j])):
                    wList[i][j][k] = round(wList[i][j][k] + random.choice([0.0,0.0,0.0,0.0,random.uniform(-1,1),random.uniform(-1,1)*error]),3)
    return wList
...
def trainSine(n): # generates training data for the neural network
    ...
    while a+b < batch:
        ...
        if x > 0.25 and x < 0.75:
            if y < (math.sqrt( -(4*x-2)**2 + 1 )/4)+1/2 and y > -(math.sqrt( -(4*x-2)**2 + 1 )/4)+1/2: #  math.sin(math.pi*x):
                if a > (batch/2)-1:
                    continue
                else:
                    xValsAbove.append(x)
...
def plotter(): # test for showing how the data points should be classified
    ...
    amount = 280 # amount of points to plot (should be high, so we can see what the classification should look like)
    ...
    while e < 1:
        sineX.append(e)
        sineY.append(math.sin(math.pi*e))
        e += 0.001
